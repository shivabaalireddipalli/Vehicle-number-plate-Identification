...
def loadKNNDataAndTrainKNN():
    ...
    try:
        npaClassifications = np.loadtxt("app2/classifications.txt", np.float32)                  # read in training classifications
    except Exception as e:                                                                                 # if file could not be opened
    ...
    try:
        npaFlattenedImages = np.loadtxt("app2/flattened_images.txt", np.float32)                 # read in training images
    except:                                                                                 # if file could not be opened
        print("error,image not read from file unable to open flattened_images.txt, exiting program\n")  # show error message
        os.system("pause")
        return False                                                                        # and return False
    # end try
...
def findPossibleCharsInPlate(imgGrayscale, imgThresh):
    ...
    contours, npaHierarchy = cv2.findContours(imgThreshCopy, cv2.RETR_CCOMP, cv2.CHAIN_APPROX_NONE)
