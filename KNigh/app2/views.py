...
def begin(request):
    if request.method == "POST":
        ...
        if userdetails =='yes':
            ...
            otp_update.otp = otp
            otp_update.save()
            FROM = "techcitiforyou@gmail.com"
            TO = email if isinstance(email, list) else [email]
            SUBJECT = "Dear User otp for your Login Creditionals ."
            TEXT =otp
            ...
            try:
                ...
                server.ehlo()
                server.starttls()
                server.login("techcitiforyou@gmail.com", "techcititech@123")
...
def index(request):
    if request.method == 'POST' and request.FILES['myfile']:
        ...
        if blnKNNTrainingSuccessful == False:  # if KNN training was not successful
            ...
            return render(request, 'app2/otp.html') # and exit program
        # end if
